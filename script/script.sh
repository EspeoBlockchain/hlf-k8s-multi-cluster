...
KUBECONFIG=$CONFIG_CLUSTER1 helm repo add kfs https://kfsoftware.github.io/hlf-helm-charts --force-update  &
KUBECONFIG=$CONFIG_CLUSTER1 helm install hlf-operator --version=1.7.0 kfs/hlf-operator &
KUBECONFIG=$CONFIG_CLUSTER1 istioctl install -y &
KUBECONFIG=$CONFIG_CLUSTER2 helm repo add kfs https://kfsoftware.github.io/hlf-helm-charts --force-update  &
KUBECONFIG=$CONFIG_CLUSTER2 helm install hlf-operator --version=1.7.0 kfs/hlf-operator &
...
# Deploys for ORG1
export KUBECONFIG=$CONFIG_CLUSTER1
deploy_peer $ORG1_CA $ORG1_PEER0_USERNAME $ORG1_PEER0_PASSWORD $ORG1_ENROLL_ID $ORG1_ENROLL_SECRET $ORG1 $PEER0_ORG1 $PEER0_ORG1_DOMAIN resources/org1/peer0.yaml
deploy_peer $ORG1_CA $ORG1_PEER1_USERNAME $ORG1_PEER1_PASSWORD $ORG1_ENROLL_ID $ORG1_ENROLL_SECRET $ORG1 $PEER1_ORG1 $PEER1_ORG1_DOMAIN resources/org1/peer1.yaml
deploy_orderer $ORDERER_CA $ORDERER1_ORDERER_USERNAME $ORDERER1_ORDERER_PASSWORD $ORDERER_ENROLL_ID $ORDERER_ENROLL_SECRET $ORGORD $ORD1_NODE $ORD1_DOMAIN $ORDERER1_DEPLOYMENT
# Deploys for ORG2
export KUBECONFIG=$CONFIG_CLUSTER2
deploy_peer $ORG2_CA $ORG2_PEER0_USERNAME $ORG2_PEER0_PASSWORD $ORG2_ENROLL_ID $ORG2_ENROLL_SECRET $ORG2 $PEER0_ORG2 $PEER0_ORG2_DOMAIN resources/org2/peer0.yaml
deploy_peer $ORG2_CA $ORG2_PEER1_USERNAME $ORG2_PEER1_PASSWORD $ORG2_ENROLL_ID $ORG2_ENROLL_SECRET $ORG2 $PEER1_ORG2 $PEER1_ORG2_DOMAIN resources/org2/peer1.yaml
deploy_orderer $ORG2_CA $ORDERER2_ORDERER_USERNAME $ORDERER2_ORDERER_PASSWORD $ORDERER_ENROLL_ID $ORDERER_ENROLL_SECRET $ORGORD $ORD2_NODE $ORD2_DOMAIN $ORDERER2_DEPLOYMENT
KUBECONFIG=$CONFIG_CLUSTER1 wait_for_fabric
KUBECONFIG=$CONFIG_CLUSTER2 wait_for_fabric
...
#Fix Orderer config bug
yq -i '.peers = {}' $ORDERER1_CONFIG
...
sleep 5
export KUBECONFIG=$CONFIG_CLUSTER1
inspect_channel $ORG1_CONFIG $ORG1_ADMIN_USERNAME ${PEER0_ORG1}.default
...
kubectl hlf channel signupdate --channel ${CHANNEL_NAME} -f config_update_in_envelope.pb --user $ORDERER1_ADMIN_USERNAME --config $ORDERER1_CONFIG --mspid $ORGORD --output ord-demo-update-sign.pb
...
wait
SEQUENCE=1
KUBECONFIG=$CONFIG_CLUSTER1 approve_chaincode $ORG1_CONFIG $ORG1_ADMIN_USERNAME $PEER0_ORG1 $SEQUENCE
KUBECONFIG=$CONFIG_CLUSTER2 approve_chaincode $ORG2_CONFIG $ORG2_ADMIN_USERNAME $PEER0_ORG2 $SEQUENCE
...
KUBECONFIG=$CONFIG_CLUSTER1 kubectl hlf chaincode invoke --config $ORG1_CONFIG --user $ORG1_ADMIN_USERNAME --peer ${PEER0_ORG1}.default --chaincode $CHAINCODE_LABEL --channel $CHANNEL_NAME --fcn createCar --args 'CAR10' --args 'Mazda' --args 'MX5' --args 'black' --args 'Karol'
KUBECONFIG=$CONFIG_CLUSTER2 kubectl hlf chaincode invoke --config $ORG2_CONFIG --user $ORG2_ADMIN_USERNAME --peer ${PEER0_ORG2}.default --chaincode $CHAINCODE_LABEL --channel $CHANNEL_NAME --fcn createCar --args 'CAR11' --args 'Seat' --args 'Leon' --args 'black' --args 'Matt'

KUBECONFIG=$CONFIG_CLUSTER1 kubectl hlf chaincode query --config $ORG1_CONFIG --user $ORG1_ADMIN_USERNAME --peer ${PEER0_ORG1}.default --chaincode $CHAINCODE_LABEL --channel $CHANNEL_NAME --fcn queryCar --args 'CAR11' | jq
KUBECONFIG=$CONFIG_CLUSTER2 kubectl hlf chaincode query --config $ORG2_CONFIG --user $ORG2_ADMIN_USERNAME --peer ${PEER0_ORG2}.default --chaincode $CHAINCODE_LABEL --channel $CHANNEL_NAME --fcn queryCar --args 'CAR10' | jq
